...
def fetch_and_chain(upi_id):
    ...
    try:
        response_vpa = requests.post(HALFBLOOD_URL, data=json.dumps(vpa_payload),
        ...
        if not vpa_info:
            return {
                "error": "'verify_vpa_resp' object not found in FamPay response.",
                "developer": "@istgrehu"
            }, 400
        ...
        return {
            "error": f"FamPay API call failed: {str(e)}",
            "developer": "@istgrehu"
        }, 500

    # Step 2: Razorpay IFSC lookup
    final_output = {
    ...
    if ifsc_code:
        try:
            ...
            if response_ifsc.status_code == 200:
                ...
                final_output["bank_details_raw"] = {
                    "warning": f"Razorpay returned status {response_ifsc.status_code}"
                }
        
        except requests.exceptions.RequestException as e:
            final_output["bank_details_raw"] = {
                "warning": f"Error during Razorpay API call: {str(e)}"
            }
...
def api_upi_lookup():
    ...
    if not upi_id:
        return jsonify({
            "error": "Missing required parameter: upi_id",
            "developer": "@istgrehu"
        }), 400
